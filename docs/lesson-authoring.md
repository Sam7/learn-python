...
- `planning`: saves learner-owned structured notes such as inputs/process/outputs or project responsibilities. Required fields gate continuation only when non-blank; their meaning is never graded. Use this when the learner should make a design decision in their own words without pretending automated text interpretation is reliable.
...
Use a code challenge only when changing code is part of the learning objective. For every `code` activity presented as a challenge, check that running `starterCode` unchanged does **not** complete it. The starter should give the learner a useful foothold, but require them to add, change, repair, or meaningfully adapt code before passing. Avoid “press Run to prove this already-finished program works” tasks: use prediction, trace, choice, or an explicitly observational step instead. In the browser journey, verify that the unchanged starter leaves progression gated, then verify that a learner-edited solution passes.
