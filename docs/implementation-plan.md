...
- Curriculum stages and lesson data are owned by `src/curriculum/stages/`; generic learning components render activities while progression and assessment operate on typed definitions. Stages 0–5 (53 lessons) are ready. Later stage/lesson IDs and titles mirror the source specification and remain unavailable until their full learning journeys are authored.
...
Stages 1–5 are fully authored in their own curriculum files. Continue with Stage 6 — Functions and Abstraction. Add focused AST requirements for function definition, calls, parameters, and return only as each concept is introduced; ensure behavior checks distinguish returning a value from printing one. Reuse the generic code, output, behavior, trace, and choice activities rather than creating function-specific page components.
...
Author one complete stage at a time. Tests must cover data integrity, every task's completion contract, behavior validators, persistence, and at least one real-browser Python journey per stage. Mark a stage ready only after its full chapter test and desktop/iPad visual review pass. Next active batch: Stage 6 — Functions and Abstraction.
