...
- Curriculum stages and lesson data are owned by `src/curriculum/stages/`; generic learning components render activities while progression and assessment operate on typed definitions. Stages 0–8 (81 lessons) are ready. Later stage/lesson IDs and titles mirror the source specification and remain unavailable until their full learning journeys are authored.
...
- The pre-specification eight-module catalogue is already removed. The nine currently available items are canonical Stages 0–8 and must remain; deleting them would remove the first 81 lessons of `docs/curriculum-01.md`.
...
- Stage 9 should initially reuse existing error display, output evidence, trace tables, Python assertions, and behavior cases. Do not grade free-form debugging explanations for semantic correctness. Add a reusable evidence/hypothesis interaction only if the chapter cannot express its required learner action with existing activities.
- Stage 10 has a real prospective gap: learners need to fill Inputs / Process / Outputs and planning information, while current `reflection` is deliberately optional and ungraded. Before authoring those lessons, design one generic structured planning activity with stable field IDs, progress persistence, keyboard/touch usability, and completion based on required fields—not guessed semantic correctness. Projects should then compose ordinary code activities and behavior acceptance cases.
- Stage 11 requires a distinct file-workspace capability, not a lesson-specific editor: named files, a designated entry point, persistent per-file progress, safe runtime reset/recovery, and clear virtual-filesystem boundaries. Before the file lessons, prove Python module imports and filesystem persistence across separate worker runs, then design the smallest generic file-workspace component and progress schema.

## Next stage and capability gates

Stages 1–8 are fully authored in their own curriculum files. Continue with Stage 9 — Debugging and Correctness. Reuse generic activities unless a concrete interaction gap is identified; any new `code` challenge must require a meaningful learner edit from its starter.
...
Author one complete stage at a time. Tests must cover data integrity, every task's completion contract, behavior validators, persistence, and at least one real-browser Python journey per stage. Mark a stage ready only after its full chapter test and desktop/iPad visual review pass. Next active batch: Stage 9 — Debugging and Correctness.
