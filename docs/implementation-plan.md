...
- [x] Stage 11 — Connecting Programming to the Real World (eight lessons)
...
- Curriculum stages and lesson data are owned by `src/curriculum/stages/`; generic learning components render activities while progression and assessment operate on typed definitions. Stages 0–11 (109 lessons) are ready.
...
- Progress schema v3 stores lesson/step position and each activity's code, response, hint count, completion, or bounded virtual project files. V1 lesson completion/code and v2 activity progress normalize to the current schema.
...
- Stages 0–10 use generic activity primitives with focused extensions where a real learning interaction required them: expression evaluation, learner-filled trace tables, branch traces, collection and structured-data AST checks, timeouts, debugging evidence, and learner-owned planning. Stage 11 adds bounded multi-file workspaces as a reusable activity kind. Code challenges require meaningful edits; prediction, trace, choice, and planning cover finished-code observation. Runtime initialization failures appear in the sticky action area with useful detail.
...
- Stage 11 filesystem spike: a real browser worker writes `score.txt` and `scores.py`, a later run imports the module and reads the saved value, and `PythonRunner.reset()` clears both. This proved that durable learner progress must pass explicit file snapshots into and out of each run; warm-worker state is not the persistence authority.
- Stage 11 workspace: run requests carry a bounded virtual-file snapshot; the worker seeds its filesystem for each run and returns the resulting files for progress persistence. File names and sizes are constrained, unrelated learner `.py` files remain available for imports, and activity fixtures isolate repeatable behavior cases.
- Stage 11 verification: all 116 unit tests pass; lint and TypeScript/production build pass; all 36 Playwright tests pass. Chromium completes all eight lessons, verifies each code challenge's unchanged starter stays gated, and covers random behavior, file persistence, JSON records, expected missing-file handling, project modules, and the capstone. Stage 10 now verifies unlocking and restoring Stage 11. WebKit iPad landscape and portrait workspace journeys pass rotation, refresh, output, and file restoration checks. Desktop and both iPad screenshots were captured and inspected. Vite retains its non-blocking bundle-size advisory.

## Architecture review for the remaining curriculum

- The pre-specification eight-module catalogue is already removed. The available items are canonical Stages 0–11 and must remain; deleting them would remove the curriculum in `docs/curriculum-01.md`.
...
- Stage 11 uses a distinct, reusable file-workspace capability, not a lesson-specific editor: named files, a designated entry point, per-file progress, safe runtime reset/recovery, and an explicit virtual-filesystem lifecycle. It was proven across separate worker runs before authoring the file lessons.

## Next stage and capability gates

Stages 0–11 are fully authored in their own curriculum files. Reuse generic activities unless a concrete interaction gap is identified; use a code challenge only when the learner must make a meaningful edit. If the task is to inspect, predict, trace, classify, or reason about unchanged code, use a non-code activity instead.
...
- Before Stage 11: scoped multi-file editing and persistent virtual files, with explicit run/reset/recovery boundaries. Prove Pyodide filesystem persistence before authoring the disk lesson. (Complete.)

Author one complete stage at a time. Tests must cover data integrity, every task's completion contract, behavior validators, persistence, and at least one real-browser Python journey per stage. Mark a stage ready only after its full chapter test and desktop/iPad visual review pass. All curriculum stages are currently authored; future additions should follow this same stage-sized verification process.
