...
- Stage 11 filesystem spike: a real browser worker writes `score.txt` and `scores.py`, a later run imports the module and reads the saved value, and `PythonRunner.reset()` clears both. The future workspace must therefore pass an explicit file snapshot with each run and return updated files to persisted learner progress; warm-worker state is not its durability mechanism.
