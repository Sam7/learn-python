...
test('Stages 0–2: the first three curriculum stages complete and unlock Decisions', async ({ page, browserName }, testInfo) => {
  ...
  await runAndExpectPass(page, 'print("My name is Byte")\nprint("I live on Mars")\nprint("I like pancakes")')
  ...
  await expect(page.getByRole('button', { name: 'Next lesson' })).toBeDisabled()
  await runAndExpectPass(page, 'score = 10\nprint(score)')
  await page.getByRole('button', { name: 'Next lesson' }).click()
  ...
  await page.getByRole('button', { name: 'Next step' }).click()
  await runUnchangedStarterAndExpectBlocked(page)
  await runAndExpectPass(page, 'answer = 7 + 8\nprint(answer)')
  await page.getByRole('button', { name: 'Next lesson' }).click()

  await page.getByRole('button', { name: /price and quantity/ }).click()
  await page.getByRole('button', { name: 'Next step' }).click()
  await runUnchangedStarterAndExpectBlocked(page)
  await runAndExpectPass(page, 'price = 12\nquantity = 4\nprint(price * quantity)')
  await page.getByRole('button', { name: 'Next lesson' }).click()
  ...
  await runUnchangedStarterAndExpectBlocked(page, ['Sam'])
  await runAndExpectPass(page, 'name = input("What is your name? ")\nprint("Hello", name)', ['Sam'])
  ...
  await expect(page.getByRole('status')).toContainText('Python raised the expected TypeError.')
  await expect(output).toContainText('TypeError:')
  await page.getByRole('button', { name: 'Next lesson' }).click()

  await runUnchangedStarterAndExpectBlocked(page, ['12'])
  await expect(output).toContainText('TypeError:')
  await runAndExpectPass(page, 'age = int(input("How old are you? "))\nprint(age + 1)', ['12'])
  await expect(page.getByRole('status')).toContainText('works with different answers', { timeout: 20_000 })
  await page.getByRole('button', { name: 'Next lesson' }).click()

  await runUnchangedStarterAndExpectBlocked(page, ['Maya', '12'])
  await runAndExpectPass(page, 'name = input("What is your name? ")\nage_text = input("How old are you? ")\nage = int(age_text)\nprint("Hi", name + "!")\nprint("Next year you will be", age + 1)\nprint("In five years you will be", age + 5)', ['Maya', '12'])
  await expect(page.getByRole('status')).toContainText('works with different answers', { timeout: 20_000 })
