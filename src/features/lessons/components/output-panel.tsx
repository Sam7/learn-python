...
export function OutputPanel({ execution, isRunning, validationMessage = null, embedded = false }: OutputPanelProps) {
  ...
  return (
    <section className={cn('overflow-hidden bg-white', !embedded && 'rounded-2xl border border-line')} aria-live="polite" aria-label="Python output">
      {!embedded ? (
      ...
      <div className={cn('min-h-[112px] p-5', isError && 'bg-coral/[0.035]')}>
        ...
        {validationMessage ? (
          <div className={cn('mt-4 rounded-xl border px-4 py-3 text-sm leading-6', validationMessage.passed ? 'border-teal/25 bg-mist text-teal-dark' : 'border-coral/25 bg-coral/5 text-coral')} role="status">
